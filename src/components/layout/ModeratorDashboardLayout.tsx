...
export default function ModeratorDashboardLayout() {
  ...
  const checkUser = async () => {
    try {
      ...
      if (modSession) {
        ...
        if (import.meta.env.VITE_SUPABASE_URL && import.meta.env.VITE_SUPABASE_URL !== 'https://placeholder.supabase.co' && mod.email) {
          try {
            const { data } = await supabase.from('moderators').select('avatar_url').eq('email', mod.email).maybeSingle();
            if (data?.avatar_url) {
              mod.avatar_url = data.avatar_url;
              localStorage.setItem('unixx_moderator_session', JSON.stringify(mod));
              localStorage.setItem(`unixx_avatar_${mod.email}`, data.avatar_url);
            }
          } catch (e) {}
        }
        setUser(mod);
  ...
  const navigation = [
    ...
    { name: 'Manage Students', href: '/moderator-dashboard/students', icon: Users },
    { name: 'Manage Faculty', href: '/moderator-dashboard/faculty', icon: Users },
    { name: 'Departments', href: '/moderator-dashboard/departments', icon: BookOpen },
    { name: 'Study Groups & Chat', href: '/moderator-dashboard/chat', icon: MessageSquare },
    { name: 'Manage Notices', href: '/moderator-dashboard/notices', icon: Bell },
    { name: 'Routines', href: '/moderator-dashboard/routines', icon: Calendar },
    { name: 'Attendance', href: '/moderator-dashboard/attendance', icon: CheckSquare },
    { name: 'Analytics', href: '/moderator-dashboard/analytics', icon: GraduationCap },
    { name: 'Settings', href: '/moderator-dashboard/settings', icon: Settings },
  ];
  ...
  return (
    <div className="h-screen font-sans flex relative overflow-hidden bg-slate-50">
      ...
      <div className={cn(
        ...
        <div className={cn(
          "h-20 flex items-center bg-white/40 dark:bg-slate-900/50 backdrop-blur-3xl border-b border-white/50 dark:border-white/10 relative before:absolute before:inset-x-0 before:top-0 before:h-[3px] before:bg-gradient-to-r before:from-purple-500 before:via-violet-500 before:via-pink-500 before:to-emerald-400 before:z-20 before:shadow-[0_0_12px_rgba(168,85,247,0.8)] after:absolute after:inset-x-0 after:top-[3px] after:h-[1px] after:bg-gradient-to-r after:from-transparent after:via-white/80 dark:after:via-white/20 after:to-transparent after:z-20",
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-none">
            <div className="absolute -top-10 -left-10 w-36 h-36 bg-purple-400/20 dark:bg-purple-500/10 rounded-full blur-2xl pointer-events-none" />
            <div className="absolute -top-10 -right-10 w-36 h-36 bg-emerald-400/20 dark:bg-emerald-500/10 rounded-full blur-2xl pointer-events-none" />
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 bg-[#f8fafc] dark:bg-slate-950">
        <header className="h-20 bg-white/40 dark:bg-slate-900/50 backdrop-blur-3xl border-b border-white/50 dark:border-white/10 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-30 shadow-[0_4px_30px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_30px_rgba(0,0,0,0.3)] relative before:absolute before:inset-x-0 before:top-0 before:h-[3px] before:bg-gradient-to-r before:from-purple-500 before:via-violet-500 before:via-pink-500 before:to-emerald-400 before:z-20 before:shadow-[0_0_12px_rgba(168,85,247,0.8)] after:absolute after:inset-x-0 after:top-[3px] after:h-[1px] after:bg-gradient-to-r after:from-transparent after:via-white/80 dark:after:via-white/20 after:to-transparent after:z-20">
          {/* Liquid Ambient Glowing Light Orbs */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-none">
            <div className="absolute -top-12 -left-12 w-56 h-56 bg-purple-400/20 dark:bg-purple-500/10 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute -top-16 left-1/3 w-72 h-72 bg-violet-500/15 dark:bg-pink-600/15 rounded-full blur-3xl pointer-events-none" />
            <div className="absolute -top-12 -right-12 w-56 h-56 bg-emerald-400/15 dark:bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />
          </div>
          ...
          <div className="flex items-center gap-2 relative z-10">
            ...
            {location.pathname !== '/moderator-dashboard' && location.pathname !== '/' && (
            ...
            <Link to="/moderator-dashboard" className="h-10 sm:h-11 flex items-center justify-center -ml-1 md:ml-0 relative group px-1 py-0.5">
            ...
            <div className="hidden lg:flex items-center ml-2">
              <span className="px-3.5 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider bg-purple-500/10 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-300/70 dark:border-purple-700/60 backdrop-blur-md shadow-2xs flex items-center gap-1.5">
            ...
            <div className="hidden md:flex relative w-72 lg:w-80 ml-3">
              ...
              <input 
                type="text" 
                placeholder="Search groups, users, logs..." 
                className="w-full pl-10 pr-16 py-2.5 bg-white/40 dark:bg-slate-800/40 backdrop-blur-md border border-white/60 dark:border-slate-700/60 rounded-xl text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-500/80 transition-all shadow-inner placeholder:text-slate-500 dark:placeholder:text-slate-400"
              />
          ...
          <div className="flex items-center gap-2 sm:gap-3.5 relative z-10">
            ...
            <HeaderMessageMenu user={user} role="moderator" />
            <HeaderNotificationMenu user={user} role="moderator" />
            <ThemeToggle />
            <UserProfileDropdown user={user} role="moderator" handleSignOut={handleSignOut} />
          </div>
        </header>
        ...
        <MobileBottomNav role="moderator" navigation={navigation} handleSignOut={handleSignOut} user={user} />
      </div>
    </div>
  );
}
