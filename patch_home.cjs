...
const target = `              <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4">{selectedNotice.title}</h3>
              <div className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">
                {selectedNotice.content}
              </div>`;

const replacement = `              <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4">{selectedNotice.title}</h3>
              {selectedNotice.type === 'image' ? (
                <div className="rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 mb-5 border border-slate-200 dark:border-slate-800 relative group">
                  {selectedNotice.content.startsWith('data:application/pdf') ? (
                  ...
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <a 
                      ...
                      download={selectedNotice.title.replace(/\\s+/g, '_') + (selectedNotice.content.startsWith('data:application/pdf') ? '.pdf' : '.png')}
